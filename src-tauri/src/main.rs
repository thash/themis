...
#[tauri::command]
fn rename_files_with_text_replacement(file_paths: Vec<String>, find_text: String, replace_text: String) -> RenameResult {
    let mut result = RenameResult {
        success: true,
        renamed: 0,
        errors: Vec::new(),
    ...
    for file_path in file_paths {
        ...
        if let (Some(parent), Some(file_name)) = (path.parent(), path.file_name()) {
            ...
            // Skip if the file name doesn't contain the text to find
            if !file_name_str.contains(&find_text) {
                continue;
            }

            // Create the new file name by replacing the text
            let new_file_name = file_name_str.replace(&find_text, &replace_text);
            let new_path = parent.join(new_file_name);
...
fn main() {
    tauri::Builder::default()
        .invoke_handler(tauri::generate_handler![
            rename_files_with_prefix_suffix,
            rename_files_with_text_replacement
        ])
